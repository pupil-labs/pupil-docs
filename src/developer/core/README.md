...
Welcome to the developer documentation for Pupil Core. If you have not already, we
highly recommend to read the [_Getting Started_](/core/#getting-started) section, as
well as the [_User Guide_](/core/software/pupil-capture.html), before continuing to
...
There are various ways to interact with the Pupil Core documentation as a developer,
...
The Pupil Core [Network API](#network-api) allows you to remote control the Pupil Core
software, as well as receive and send data to it in realtime. It provides access to
nearly all data generated by the Pupil Core software, e.g. gaze, fixation, and video data. 

The API can be used with any programming language that supports
...
[3d pointcloud data](https://gist.github.com/papr/0f13943e2aebd768ab6b1508d466caae)
generated by the RealSense 3D video backend.

##### Changing The Existing Code Base
If you want to make changes to the code base it is required to run the
[Pupil Core software from source](#running-from-source). 

Please be aware, that most use cases can be satisfied with one of the approaches above
...
1. Pupil Remote - Simple text-based API, entrypoint to second stage
2. The IPC Backbone - [msgpack](https://msgpack.org/) based API, access to realtime data
...
We highly recommend to read [Chapter 2 of the ZeroMQ guide](http://zguide.zeromq.org/php:chapter2)
to get an intuition on the philosophy behind these socket types.

The Pupil Core software uses the [pyzmq](https://github.com/zeromq/pyzmq) module, a
...
volume. If you find that you need to write more that 10 messages a second, it is
...
The Pupil Core software uses a PUB-SUB Proxy as their messaging bus. We call it the
...
actors' messages. Therefore, it is the backbone of all communication from, to, and
